...
void solve_maze(char *path, int start_row, int start_col, int end_row, int end_col) {
    ...
    int shortest_path = bfs(right_walls, bottom_walls, start_row, start_col, end_row, end_col, rows, cols, prev);
    
    if (shortest_path != -1) {
            mark_solution_path(prev, solution_path, start_row, start_col, end_row, end_col);
...
int bfs(int **right_walls, int **bottom_walls, int start_row, int start_col, int end_row, int end_col, int rows, int cols, Point prev[MAX_ROWS][MAX_COLS]) {
    ...
    while (!is_empty(&q)) {
        ...
        // right
        if (current.col + 1 < cols && !right_walls[current.row][current.col] && !visited[current.row][current.col + 1]) {
        ...
        // left
        if (current.col - 1 >= 0 && !right_walls[current.row][current.col - 1] && !visited[current.row][current.col - 1]) {
            Point next = {current.row, current.col - 1, current.dist + 1};
            enqueue(&q, next);
            visited[current.row][current.col - 1] = true;
            prev[current.row][current.col - 1] = current;
        ...
        // down
        if (current.row + 1 < rows && !bottom_walls[current.row][current.col] && !visited[current.row + 1][current.col]) {
        ...
        // up
        if (current.row - 1 >= 0 && !bottom_walls[current.row - 1][current.col] && !visited[current.row - 1][current.col]) {
            Point next = {current.row - 1, current.col, current.dist + 1};
            enqueue(&q, next);
            visited[current.row - 1][current.col] = true;
            prev[current.row - 1][current.col] = current;
...
void mark_solution_path(Point prev[MAX_ROWS][MAX_COLS], bool solution_path[MAX_ROWS][MAX_COLS], int start_row, int start_col, int end_row, int end_col) {
    ...
    mark_solution_path(prev, solution_path, start_row, start_col, prev[end_row][end_col].row, prev[end_row][end_col].col);
